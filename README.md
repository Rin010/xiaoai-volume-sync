...
| 小爱同学使用媒体音量 | 小爱自身选择媒体流 3 来读档位和创建播放器；遇到显式 USAGE_ASSISTANT 播放对象也改为媒体用途。它原先静音媒体和自动抬高媒体音量的内部操作会被拦截。 |
...
1. 安装本目录对应的 `XiaoAiVolumeSync-1.2.1.apk`。
...
实测设备包括小米平板 M2105K81AC / elish（HyperOS OS3.0.6.0.WNXCNXM，小爱 7.13.33.0017 / 507013033）和小米 23116PN5BC / shennong（HyperOS OS3.0.306.0.WNBCNXM，小爱 7.13.21.0017 / 507013021），均为 Android 16 / API 36。直接使用媒体流的混淆类 Hook 只对这两个已检查版本启用；其他版本需要重新检查。最低 Android 12 / API 31 并不代表已在其他设备验证。

手机版 7.13.21 将流工具类从平板版的 `k00.o` 改为 `d00.o`。1.2.1 为两版设置显式映射，并在手机内验证流选择、媒体静音拦截、最低音量拦截和实际 AudioTrack 属性。
...
- `DirectMediaHooks.java` 把小爱的共享流选择函数改为媒体流，保护识别时静音和自动改媒体音量的路径，并处理 AudioTrack / MediaPlayer 的显式助手播放属性。
...
./build.ps1 -JavaHome 'C:/path/to/jdk' `
  ...
  -OutputApk 'C:/path/to/XiaoAiVolumeSync-1.2.1.apk'
```
